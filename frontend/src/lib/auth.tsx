...
interface AuthCtx {
  ...
  registerEmail: (email: string, password: string, name: string, sex?: string, referralCode?: string) => Promise<void>;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const registerEmail = useCallback(async (email: string, password: string, name: string, sex?: string, referralCode?: string) => {
    const r = await fetch(`${API}/api/auth/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password, display_name: name, sex, referral_code: referralCode || undefined }),
    });
    if (!r.ok) {
      const err = await r.json().catch(() => ({}));
